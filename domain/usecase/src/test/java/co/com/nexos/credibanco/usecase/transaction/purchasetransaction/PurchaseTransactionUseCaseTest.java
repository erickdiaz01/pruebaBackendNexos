...
class PurchaseTransactionUseCaseTest {
        ...
        void doTransaction() {
                ...
                when(transactionRepository.doPurchase("1000011234567890", 1200)).thenReturn(Mono.just(transaction));

                Mono<Transaction> result = purchaseTransactionUseCase.doTransaction("1000011234567890", 1200);

                StepVerifier.create(result)
                                .expectNextMatches(
                                                transactionTested -> transactionTested.getTransactionId()
                                                                .equals(transaction.getTransactionId())
                                                                && transactionTested.getCard().getCardId()
